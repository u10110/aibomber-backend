...
from apps.home.services.gpt_assistant import create_message_pca_embedding
                ...
                for chat in chats:
                    if chat:
                       ...
                       #     chat.save()
                        ...
                        messages = ChatMessages.objects.filter(chat_id_id=90).order_by('created_at')
                        message_text_array = []
                        if len(messages) > 5:
                            for message in messages:
                                message_text_array.append(message.user_message)


                            print(message_text_array)
                            embedding = create_message_pca_embedding(message_text_array)
                            print(embedding)
                            break
